...
const renderColorReference = data => {
  const reference = data.colorReference;
  if (!reference?.image) return '';
  return `<section class="color-reference" aria-label="Referencia del color"><div><p class="eyebrow">REFERENCIA DEL COLOR</p><strong>Otro producto en el color ${escapeHtml(data.color || reference.color)}.</strong><p>El tono puede variar según el material y la pantalla.</p></div><img src="${escapeHtml(reference.image)}" alt="Referencia del color ${escapeHtml(reference.color)} de otro producto"><small>STYLE ${escapeHtml(reference.style || 'No disponible')} · ${escapeHtml(reference.imageSource || 'Fuente exacta')}</small></section>`;
};
