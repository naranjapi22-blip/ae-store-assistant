...
const renderCatalogItems = items => items.map(item => `<article class="catalog-card"><button class="catalog-card-button" type="button" data-catalog-barcode="${escapeHtml(item.barcode)}"><div class="catalog-card-image" style="position:relative">${item.image ? `${item.imageIsReference ? '<div style="position:absolute;top:8px;left:8px;right:8px;z-index:1;padding:8px 10px;border:1px solid #d8cbd0;border-radius:9px;background:#fff;text-align:left"><strong style="display:block;color:#7e3650;font-size:12px">Imagen de referencia</strong><span style="display:block;margin-top:2px;color:#241f22;font-size:11px">Mismo modelo en otro color/estampado.</span></div>' : ''}<img src="${escapeHtml(item.image)}" alt="Imagen de ${escapeHtml(item.description)}"><span class="placeholder" hidden>Imagen no disponible</span>` : '<span class="placeholder">Imagen no disponible</span>'}</div><div class="catalog-card-details"><p class="eyebrow">${escapeHtml(item.style || 'STYLE no disponible')}</p><h3>${escapeHtml(item.description || 'Producto VS')}</h3><p><strong>${escapeHtml(item.color || 'Color no disponible')}</strong></p><span>${escapeHtml(item.stock)} unidades · ${escapeHtml(item.availableSizes)} tallas</span></div></button></article>`).join('');
